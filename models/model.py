...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self,
                 ...
                 batch_first=False):
        """
        customized wrapper for multi_head self-attention layer
        """

        super().__init__()
        self.attn = torch.nn.MultiheadAttention(embed_dim,
                                                num_heads,
                                                dropout,
                                                bias,
                                                add_bias_kv,
    ...
    def forward(self, x, key_padding_mask=None,
        ...
        return self.attn(x, x, x, key_padding_mask, need_weights, attn_mask)[0]
...
class LinearCRFEncoder(nn.Module):
    ...
    """
    def __init__(self, insize, n_base, state_len, bias=True, scale=5.0,  blank_score=2.0, expand_blanks=True, permute=None):
        super().__init__()
        self.scale = scale
        self.n_base = n_base
...
class CRF_encoder(nn.Module):
    def __init__(self, n_hid : int = 768, expand_blanks=True, dropout : float = 0.1):
        super(CRF_encoder, self).__init__()
        self.encoder = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=4, kernel_size=5, stride=1, padding=2),
            nn.SiLU(),
            nn.Conv1d(in_channels=4, out_channels=16, kernel_size=5, stride=1, padding=2),
            nn.SiLU(),
            nn.Conv1d(in_channels=16, out_channels=n_hid, kernel_size=19, stride=5, padding=9),
            nn.SiLU(),
            Permute(2, 0, 1), # (N, C, T) -> (T, N, C)
            # MultiHeadSelfAttention(n_hid, 8, batch_first=False),
            ReverseLSTM(n_hid, n_hid, batch_first=False, reverse=True),
            ReverseLSTM(n_hid, n_hid, batch_first=False, reverse=False),
            ReverseLSTM(n_hid, n_hid, batch_first=False, reverse=True),
            ReverseLSTM(n_hid, n_hid, batch_first=False, reverse=False),
            ReverseLSTM(n_hid, n_hid, batch_first=False, reverse=True),
            LinearCRFEncoder(insize= n_hid , n_base=4, state_len=5, scale=5.0, blank_score=2.0, expand_blanks=expand_blanks),
            # nn.Linear(n_hid, 5)
        )
...
class CTC_encoder(nn.Module):
    def __init__(self, conv : list = [8,64], n_hid : int = 512, dropout : float = 0.2):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.SiLU(),
            Permute(2, 0, 1), # (N, C, T) -> (T, N, C)
            # PositionalEncoding(d_model=n_hid),
            ReverseLSTM(n_hid, n_hid, batch_first=False, reverse=True),
            ...
            # nn.Dropout(p=dropout),
            # ReverseLSTM(n_hid // 16, n_hid // 16, batch_first=False, reverse=True),
    ...
    def __init__(self, conv: list = [16, 16], n_hid: int = 512, num_tf : int = 2):
        ...
        self.tf_encoder = nn.Sequential(
            ...
            *[
            ...
            # Linformer(
            #     input_size=1200,  # Dimension 1 of the input
            #     channels=512,  # Dimension 2 of the input
            #     dim_d=None,
            ...
            #     dim_ff=512,  # Dimension in the feed forward network
            #     dropout_ff=0.1,  # Dropout for feed forward network
            #     nhead=8,  # Number of attention heads
            #     depth=num_tf,  # How many times to run the model
            ...
            #     # What level of parameter sharing to use. For more information, see below.
            #     k_reduce_by_layer=0,
            ...
            #     include_ff=True,  # Whether to include the Feed Forward layer
